...
export const CustomerCard: React.FC<CustomerCardProps> = ({ customer, onView, onEdit, onDelete, theme, lang, baseCurrency, exchangeRates, isSelected, onSelect }) => {
  ...
  const FinancialField: React.FC<{label: string, value: number, convertedValue: number | null, colorClass?: string}> = ({ label, value, convertedValue, colorClass }) => (
    <div>
        <p className={`${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'} text-xs mb-1`}>{label}</p>
        <p className={`font-semibold ${colorClass || (theme === 'dark' ? 'text-white' : 'text-slate-800')}`}>{formatCurrency(value, customer.currency)}</p>
        {convertedValue !== null && <p className={`text-xs ${theme === 'dark' ? 'text-slate-500' : 'text-slate-500'}`}>~{formatCurrency(convertedValue, baseCurrency)}</p>}
    </div>
  );

  return (
    <div className={`p-4 rounded-xl relative ${theme === 'dark' ? 'bg-gray-900 border border-gray-700' : 'bg-white border border-slate-200 shadow-sm'} transition-all duration-300 ${isSelected ? 'border-cyan-500' : 'hover:border-cyan-500/50'}`}>
      ...
      <div onClick={onView} className="cursor-pointer">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className={`${theme === 'dark' ? 'text-white' : 'text-slate-800'} font-semibold text-lg`}>{customer.name}</h4>
          ...
          <div className="relative" onClick={e => e.stopPropagation()}>
            <button onClick={() => setShowMenu(!showMenu)} onBlur={() => setTimeout(() => setShowMenu(false), 100)} className={`p-2 rounded-lg ${theme === 'dark' ? 'hover:bg-gray-800 text-slate-400' : 'hover:bg-slate-100 text-slate-500'} transition-colors`}>
              <MoreVertical size={20} />
            </button>
            {showMenu && (
              <div className={`absolute ${lang === 'ar' ? 'left-0' : 'right-0'} mt-2 w-48 rounded-lg ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-slate-200'} shadow-xl z-10`}>
                <button onClick={onView} className={`w-full px-4 py-2 text-right ${theme === 'dark' ? 'hover:bg-gray-700 text-white' : 'hover:bg-slate-100 text-slate-700'} flex items-center gap-2 rounded-t-lg`}><Eye size={16} /> عرض التفاصيل</button>
                <button onClick={onEdit} className={`w-full px-4 py-2 text-right ${theme === 'dark' ? 'hover:bg-gray-700 text-white' : 'hover:bg-slate-100 text-slate-700'} flex items-center gap-2`}><Edit2 size={16} /> تعديل</button>
        ...
        <div className={`grid grid-cols-2 gap-3 p-3 rounded-lg ${theme === 'dark' ? 'bg-gray-800/50' : 'bg-slate-50 border-slate-200'}`}>
          ...
          <div><p className={`${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'} text-xs mb-1`}>عدد المعاملات</p><p className={`${theme === 'dark' ? 'text-purple-400' : 'text-purple-600'} font-semibold`}>{customer.totalTransactions}</p></div>
        </div>
        <div className={`mt-3 pt-3 border-t ${theme === 'dark' ? 'border-gray-700' : 'border-slate-200'} flex items-center justify-between text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
