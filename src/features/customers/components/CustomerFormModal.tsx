...
export const CustomerFormModal: React.FC<{ customer: Customer | null; onClose: () => void; onSave: (customer: Partial<Customer>) => Promise<void>; theme: AppTheme }> = ({ customer, onClose, onSave, theme }) => {
    ...
    return (
      <div className="fixed inset-0 bg-black/75 z-50" onMouseDown={onClose}>
        <div 
          ...
          className={`fixed inset-0 md:inset-auto md:left-[var(--modal-x)] md:top-[var(--modal-y)] md:w-[var(--modal-width)] md:h-[var(--modal-height)] rounded-none md:rounded-2xl shadow-2xl w-full flex flex-col ${isDark ? 'bg-gray-900 border-2 border-cyan-500/50' : 'bg-white border'}`}
        ...
        >
          <div 
            ...
            <h3 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{isEdit ? 'تعديل العميل' : 'إضافة عميل جديد'}</h3>
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="overflow-y-auto flex-1 p-6 space-y-6">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div><Label>الحالة</Label><Select value={formData.status || 'active'} onChange={(e) => setFormData({ ...formData, status: e.target.value as CustomerStatus })}><option value="active">نشط</option><option value="inactive">غير نشط</option><option value="blocked">محظور</option></Select></div>
                <div><Label>المخاطر</Label><Select value={formData.riskLevel || 'low'} onChange={(e) => setFormData({ ...formData, riskLevel: e.target.value as RiskLevel })}><option value="low">منخفض</option><option value="medium">متوسط</option><option value="high">مرتفع</option></Select></div>
                <div><Label>العملة</Label><Select value={formData.currency || 'SAR'} onChange={(e) => setFormData({ ...formData, currency: e.target.value })}><option value="SAR">ريال سعودي (SAR)</option><option value="YER">ريال يمني (YER)</option><option value="OMR">ريال عماني (OMR)</option><option value="USD">دولار أمريكي (USD)</option></Select></div>
              </div>
            </section>
          ...
          <div className={`flex justify-end gap-3 p-4 border-t mt-auto ${isDark ? 'border-gray-700' : 'border-slate-200'}`}>
            <button type="button" onClick={onClose} className={`px-6 py-3 rounded-xl ${isDark ? 'bg-gray-800' : 'bg-slate-200'} font-semibold`}>إلغاء</button>
