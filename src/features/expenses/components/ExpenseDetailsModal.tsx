...
export const ExpenseDetailsModal: React.FC<ExpenseDetailsModalProps> = ({ expense, onClose, onEdit, theme, t }) => {
  ...
  const modalBg = isDark ? `bg-gray-900 border-2 border-purple-500/50 shadow-[var(--accent-shadow)]` : 'bg-white border shadow-2xl';
  const headerBg = isDark ? 'bg-transparent' : 'bg-slate-100';
  const sectionHeaderColor = isDark ? 'text-purple-400' : 'text-purple-600';
  const detailBoxBg = isDark ? 'bg-slate-800' : 'bg-slate-50 border border-slate-200';
  ...
  const DetailItem: React.FC<{icon: React.ElementType, label: string, value?: string | React.ReactNode, fullWidth?: boolean}> = ({icon: Icon, label, value, fullWidth}) => {
    ...
    return (
      <div className={`${fullWidth ? 'md:col-span-2' : ''} flex items-start gap-3`}>
        <Icon className={`w-5 h-5 ${isDark ? 'text-gray-400' : 'text-slate-500'} mt-0.5`} />
        <div>
          <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-slate-600'}`}>{label}</p>
          <div className={`text-base font-semibold ${isDark ? 'text-gray-200' : 'text-slate-800'}`}>{value}</div>
        </div>
      </div>
    );
  };
    ...
    <div className="fixed inset-0 bg-black/75 z-50" onMouseDown={onClose}>
      ...
      >
        <div 
          ...
          className={`p-6 border-b cursor-move ${isDark ? 'border-purple-800/50' : 'border-slate-200'} ${headerBg}`}
        >
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1 flex items-center gap-3">
              <div className={`p-2 rounded-lg ${isDark ? 'bg-gray-800' : 'bg-white shadow-sm'}`}><CategoryIcon className={`w-6 h-6 ${sectionHeaderColor}`} /></div>
              <div>
                <h2 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{expense.title}</h2>
                <p className={`${isDark ? 'text-purple-200' : 'text-slate-600'} text-sm mt-1`}>{categoryConfig.label}</p>
              </div>
            </div>
            <button onClick={onClose} className={`p-2 rounded-lg transition-colors ${isDark ? 'hover:bg-white/10' : 'hover:bg-slate-200'}`}><X className="w-6 h-6" /></button>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-6">
          <div className={`p-6 rounded-xl ${isDark ? 'bg-slate-800' : 'bg-slate-100'} flex items-center justify-between`}>
            <div><p className={`text-sm ${isDark ? 'text-gray-400' : 'text-slate-600'} mb-2`}>{t.amount}</p><p className={`text-4xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{formatCurrency(expense.amount, expense.currency)}</p></div>
            <div className={`${sectionHeaderColor} ${isDark ? 'bg-purple-900' : 'bg-purple-100'} p-4 rounded-xl`}><DollarSign className="w-8 h-8" /></div>
          </div>
        ...
        <div className={`p-4 border-t ${isDark ? 'border-purple-800/50' : 'border-slate-200'} flex justify-end gap-3`}>
