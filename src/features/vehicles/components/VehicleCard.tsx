    ...
    <div className={`rounded-xl transition-all duration-300 border flex flex-col h-full ${cardClasses}`}>
      ...
      >
        <div className="flex justify-between items-start">
            ...
            <div className={`p-2 rounded-lg ${theme === 'dark' ? 'bg-gray-800 text-cyan-400' : 'bg-slate-100 text-slate-600'}`}>
                <Car size={20} />
            </div>
        </div>
        
        <div className="mt-4 space-y-2">
             <div className={`flex justify-between text-sm ${theme === 'dark' ? 'bg-gray-800/50' : 'bg-slate-50'} p-2 rounded`}>
      ...
      <div className={`p-3 border-t flex justify-between items-center gap-2 ${theme === 'dark' ? 'border-gray-700' : 'border-slate-200'}`}>
         <button 
            onClick={(e) => { e.stopPropagation(); onViewDetails(); }} 
            className={`flex items-center gap-1 px-3 py-1.5 text-xs font-bold rounded-lg ${theme === 'dark' ? 'bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20' : 'bg-cyan-50 text-cyan-600 hover:bg-cyan-100'}`}
         >
