...
import { Receipt, Wallet, Landmark, CreditCard, ArrowRightLeft, Save, Loader, FileText } from 'lucide-react';
...
export const PurchaseFooter: React.FC<PurchaseFooterProps> = ({
    invoiceType,
    notes, setNotes,
    ...
    const paymentMethodsConfig = [
        ...
        { id: 'cheque', label: 'شيك', icon: FileText, color: 'text-yellow-500', activeBg: 'bg-yellow-500/10 border-yellow-500' },
    ...
    return (
        ...
        <div className="mt-8 flex flex-col md:flex-row gap-8 items-start">
            {/* Left Side: Notes & Payment Source */}
                 ...
                 {(invoiceType === 'cash' || amountPaid > 0) && (
                    <div className={`rounded-xl border overflow-hidden transition-all duration-300 ${theme === 'dark' ? 'bg-gray-800/40 border-emerald-500/30' : 'bg-emerald-50/50 border-emerald-200 shadow-sm'}`}>
                         <div className={`px-4 py-3 border-b flex items-center gap-2 ${theme === 'dark' ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-100/50 border-emerald-200'}`}>
                            ...
                            <h4 className={`font-bold text-sm ${theme === 'dark' ? 'text-emerald-400' : 'text-emerald-800'}`}>تفاصيل دفع المبلغ للمورد</h4>
                         </div>
                        ...
                        <div className="p-4 space-y-4">
                            ...
                            <div>
                                <Label className="text-xs text-gray-500 mb-1 block">خصم من حساب (الأصول)</Label>
                                <div className="relative">
                                    <Select value={paymentAccountId} onChange={e => setPaymentAccountId(e.target.value)} className={`!text-sm !py-2 !pl-9 ${theme === 'dark' ? '!bg-gray-900' : '!bg-white'}`}>
            ...
            <div className={`w-full md:w-96 shrink-0 p-5 rounded-xl ${theme === 'dark' ? 'bg-gray-800/30 border border-gray-700' : 'bg-white border border-slate-200 shadow-sm'}`}>
                ...
                <div className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <span className="font-mono text-xl font-bold text-purple-500">{formatCurrency(calculations.total, currency)}</span>
                    </div>
        ...
        <div className={`flex justify-end items-center pt-6 mt-6 border-t ${theme === 'dark' ? 'border-gray-700' : 'border-slate-200'}`}>
            <HoloButton variant="success" icon={isSaving ? Loader : Save} onClick={onSave} disabled={isSaving} className="px-8 py-3 text-lg">
