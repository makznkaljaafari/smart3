    ...
    <div className="fixed inset-0 bg-black/75 z-50" onMouseDown={onClose}>
      ...
      >
        <div 
          ...
          className={`p-6 border-b flex items-start justify-between cursor-move ${theme === 'dark' ? 'bg-gray-900 border-gray-700' : 'bg-white border-slate-200'}`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`text-xl font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{note.title}</h2>
              <p className={`text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>{categoryConfig.label}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <section>
            ...
            <div className={`p-4 rounded-lg whitespace-pre-wrap ${theme === 'dark' ? 'bg-gray-800' : 'bg-slate-50'}`}>{note.content}</div>
          </section>
          
          <div className={`p-4 rounded-lg grid grid-cols-2 lg:grid-cols-4 gap-4 ${theme === 'dark' ? 'bg-gray-800' : 'bg-slate-50'}`}>
          ...
          {note.tags.length > 0 && (
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {note.tags.map(tag => <span key={tag} className={`px-3 py-1 text-sm rounded-full ${theme === 'dark' ? 'bg-slate-700' : 'bg-slate-100'}`}>#{tag}</span>)}
              </div>
            </section>
          )}
