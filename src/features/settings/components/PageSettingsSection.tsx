...
import { SettingsState, PageSettings, AppTheme } from '../../../types';
...
const PageConfigBlock: React.FC<PageConfigBlockProps> = ({ title, settings, onPageSizeChange, onReset, t, theme }) => {
  ...
  const tagClasses = isDark ? 'bg-gray-800 border-gray-700' : 'bg-slate-100 border-slate-300 text-slate-700';
  const inputClasses = `w-full mt-1 p-2 rounded-lg border focus:outline-none focus:ring-2 focus:ring-cyan-500 ${isDark ? 'bg-gray-800 text-white border-gray-700' : 'bg-white text-slate-800 border-slate-300'}`;
  const labelClasses = `text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`;
  ...
  return (
    <SectionBox title={t.pageSettings} theme={theme}>
      <div className="space-y-6">
        ...
        <div>
           <h4 className={`font-semibold mb-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>{t.notes}</h4>
            <div className={`p-4 rounded-lg ${isDark ? 'bg-gray-800/50' : 'bg-slate-100'} space-y-3`}>
                <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-300' : 'text-slate-700'}>{t.allowAudioNotes}</span>
                    <Toggle 
                 ...
                 <div className="flex justify-between items-center">
                    <span className={isDark ? 'text-gray-300' : 'text-slate-700'}>{t.allowImageNotes}</span>
                    <Toggle 
